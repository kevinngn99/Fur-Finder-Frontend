...
from kivy.effects.dampedscroll import DampedScrollEffect, ScrollEffect
from kivy.lang import Builder
...
<RefreshSpinner>
    AnchorLayout:
        ...
        canvas:
            Clear
            Color:
                rgba: utils.get_color_from_hex('#023b80')
            Ellipse:
                pos: self.pos
                size: self.size
        MDSpinner:
...
class RefreshSpinner(ThemableBehavior, FloatLayout):
    ...
    def set_spinner(self, *args):
        ...
        body_spinner.size = (dp(30), dp(30))
        body_spinner.y = Window.height - 20
        body_spinner.opacity = 1
        spinner = self.ids.spinner
        spinner.size = (dp(15), dp(10))
        spinner.opacity = 1
...
class RecycleView(RecycleViewBehavior, MDScrollViewRefreshLayout):
    """
